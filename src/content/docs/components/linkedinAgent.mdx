...
```
<Step>Copy the code of [Linkedin Tool](https://agentgenesis-beta.vercel.app/docs/components/linkedinTool) and paste it in `utils/linkedinTool.ts`</Step>
<Step>Copy the code of [ChatOpenAI](https://agentgenesis-beta.vercel.app/docs/components/chatOpenAi) and paste it in `utils/chatOpenAi.ts`</Step>

<Step>Add Environment Variables</Step>

```tsx title=".env" showLineNumbers {}
OPENAI_API_KEY = aSdWytUQZUkBbPpGcXNAG4X0;
...
```
Now you can customise it and use it as per choice and always build on top of it.
In this case we are getting the output in HTML table format. You can customize that by changing **outputFormat** prop of ***ChatOpenAI***.
